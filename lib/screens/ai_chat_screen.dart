...
class _AIChatScreenState extends State<AIChatScreen>
  ...
  String serverUrl = "your server url here";
  ...
  Future<void> _processAIResponse(String userMessage) async {
    ...
    try {
      ...
      final body = {'model': 'gpt-4o', 'messages': history};
    ...
    try {
      ...
      while (!isDone && pollCount < 60) {
        ...
        final statusResp = await http.get(
          Uri.parse(''), //your server url here
        );
          ...
          _ChatMessage(
            ...
            onExplainWithAI: () async {
              ...
              try {
                final url = Uri.parse(''); //your server url here
                final headers = {'Content-Type': 'application/json'};
                final explainWithAISystemPrompt =
                ...
                final body = {
                  'model': 'gpt-4o',
                  'messages': [
                    {'role': 'system', 'content': explainWithAISystemPrompt},
                    {
                      'role': 'user',
                      'content':
                          'You are a fitness coach. Here are the user\'s squat analysis results:\n$summary\nPlease provide personalized feedback and suggestions for improvement.',
                    },
                  ],
                };
                final response = await http.post(
